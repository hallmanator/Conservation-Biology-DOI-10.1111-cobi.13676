...
occurrence.file <- "yewa_Chapter4.csv"
...
observations.covariates.file = "yewa_Chapter4.csv" 
...
truncation.proportion <- 0.1*nrow(observation.data)
...
#oak_cols <- grep("oak", names(model.data), value = FALSE) # forgot to use this! Damn!For publication need to take this out...



model.data <- model.data[,-c(will_cols, urban_cols, disturbed_cols, burned_cols, vlmr_cols)]
names(model.data)
...
covs <- observation.covariates.data[,c("Julian_Dat", "Year", "Observer", "Minutes_si", "urban_highmed_utm10_165", # First add detection covariates
                                       ...
                                       "riparian2531_utm10_165", "riparian2531_utm10_315", "riparian2531_utm10_615", "riparian2531_utm10_1215", "marsh3334_utm10_165", # Now add density covariates
                                       "marsh3334_utm10_315", "marsh3334_utm10_615", "marsh3334_utm10_1215", "Pred.Occur")] 
...
dist.models$observer_date_timeXriparian165 <- update(dist.models$observer_date_time, ~. ~riparian2531_utm10_165) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXriparian165)

dist.models$observer_date_timeXriparian315 <- update(dist.models$observer_date_time, ~. ~riparian2531_utm10_315) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXriparian315)

dist.models$observer_date_timeXriparian615 <- update(dist.models$observer_date_time, ~. ~riparian2531_utm10_615) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXriparian615)

dist.models$observer_date_timeXriparian1215 <- update(dist.models$observer_date_time, ~. ~riparian2531_utm10_1215, starts = c(-1.9258, 0.0186, 3.57076, 0.00593, -0.05842, 0.21297, 0.02228, 0.02538)) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXriparian1215)

## Now going to try any cover with some oaks
dist.models$observer_date_timeXmarsh165 <- update(dist.models$observer_date_time, ~. ~marsh3334_utm10_165) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXmarsh165)

dist.models$observer_date_timeXmarsh315 <- update(dist.models$observer_date_time, ~. ~marsh3334_utm10_315) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXmarsh315)

dist.models$observer_date_timeXmarsh615 <- update(dist.models$observer_date_time, ~. ~marsh3334_utm10_615) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXmarsh615)

dist.models$observer_date_timeXmarsh1215 <- update(dist.models$observer_date_time, ~. ~marsh3334_utm10_1215) # Update the best detection model with density covariates
summary(dist.models$observer_date_timeXmarsh1215)
...
density.pred.pred.occur2 <- predict(dist.models$timeXpred.occur2, type = "state", newdata = newdat, appendData=TRUE); density.pred.pred.occur2
...
# Goodness of fit
pb.dist <- parboot(dist.models$timeXpred.occur2, fitstats, nsim=100, report=5); pb.dist
...
capture.output(summary(dist.models$timeXpred.occur2), file = file.path(output.directory,filename))

# Detection probability for all bins within the 300m radius
p.bins <- getP(dist.models$observer_date_timeXpred.occur2)
...
p.distsamp <- mean(rowSums(getP(dist.models$timeXpred.occur2))); p.distsamp

test <- predict(dist.models$timeXpred.occur2, type = "det", newdata = umf.dist, appendData=FALSE)
...
gdist.models$observer_date_time <- gdistsamp(~1, ~1, ~Observer + JulDat + Time, data=umf.gdist, keyfun="halfnorm", output="density", mixture="NB", K=10, starts = c(-1.73, 3.5509, 0.0554, -0.1047, 0.2036, 0.0605, 0.0202, 0.36))
...
## For YEWA I'm starting with Riparian
gdist.models$observer_dateXriparian165 <- update(gdist.models$observer_date, ~riparian2531_utm10_165, ~., ~.) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXriparian165)

gdist.models$observer_dateXriparian315 <- update(gdist.models$observer_date, ~riparian2531_utm10_315, ~., ~., starts = c(-1.8941, 0.0136, 3.72715, -0.03743, -0.07251, 0.16626, -0.00297, 1.14)) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXriparian315)

gdist.models$observer_dateXriparian615 <- update(gdist.models$observer_date, ~riparian2531_utm10_615, ~., ~., starts = c(-1.8941, 0.0136, 3.72715, -0.03743, -0.07251, 0.16626, -0.00297, 1.14)) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXriparian615)

gdist.models$observer_dateXriparian1215 <- update(gdist.models$observer_date, ~riparian2531_utm10_1215, ~., ~., starts = c(-1.8941, 0.0136, 3.72715, -0.03743, -0.07251, 0.16626, -0.00297, 1.14)) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXriparian1215)

## Check which radius is best for this variable when also accounting for detection
gdist.MSmodels <- modSel(fitList(fits=gdist.models));gdist.MSmodels


## Now going to try marsh
gdist.models$observer_dateXmarsh165 <- update(gdist.models$observer_date, ~marsh3334_utm10_165, ~., ~., starts = c(-1.8941, 0.0136, 3.72715, -0.03743, -0.07251, 0.16626, -0.00297, 1.14)) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXmarsh165)

gdist.models$observer_dateXmarsh315 <- update(gdist.models$observer_date, ~marsh3334_utm10_315, ~., ~., starts = c(-1.8941, 0.0136, 3.72715, -0.03743, -0.07251, 0.16626, -0.00297, 1.14)) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXmarsh315)

gdist.models$observer_dateXmarsh615 <- update(gdist.models$observer_date, ~marsh3334_utm10_615, ~., ~., starts = c(-1.8941, 0.0136, 3.72715, -0.03743, -0.07251, 0.16626, -0.00297, 1.14)) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXmarsh615)

gdist.models$observer_dateXmarsh1215 <- update(gdist.models$observer_date, ~marsh3334_utm10_1215, ~., ~., starts = c(-1.8941, 0.0136, 3.72715, -0.03743, -0.07251, 0.16626, -0.00297, 1.14)) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXmarsh1215)
...
## Now try with the predicted habitat suitability
gdist.models$observer_dateXpred.occur <- update(gdist.models$observer_date, ~Pred.Occur, ~., ~.) # Update the best detection model with density covariates
summary(gdist.models$observer_dateXpred.occur)

gdist.models$observer_dateXpred.occur2 <- update(gdist.models$observer_date, ~Pred.Occur + I(Pred.Occur^2), ~., ~.) # Update the best detection model with density covariates
...
gdist.models$observerXpred.occur2 <- update(gdist.models$observer, ~Pred.Occur + I(Pred.Occur^2), ~., ~., starts = c(-3.69, 10.87, -8.31, 3.7120, 0.1185, 0.1436, 0.2408, 12.8)) 
summary(gdist.models$observerXpred.occur2)

gdist.models$dateXpred.occur2 <- update(gdist.models$date, ~Pred.Occur + I(Pred.Occur^2), ~., ~., starts = c(-3.69, 10.87, -8.31, 3.7120, 0.0, 12.8)) 
...
gdist.models$observer_dateXpred.occur2 <- update(gdist.models$observer_date, ~Pred.Occur + I(Pred.Occur^2), ~., ~.) 
summary(gdist.models$observer_dateXpred.occur2)

gdist.models$observer_timeXpred.occur2 <- update(gdist.models$observer_time, ~Pred.Occur + I(Pred.Occur^2), ~., ~.) 
summary(gdist.models$observer_timeXpred.occur2)

gdist.models$date_timeXpred.occur2 <- update(gdist.models$date_time, ~Pred.Occur + I(Pred.Occur^2), ~., ~.) 
summary(gdist.models$date_timeXpred.occur2)

gdist.models$observer_date_timeXpred.occur2 <- update(gdist.models$observer_date_time, ~Pred.Occur + I(Pred.Occur^2), ~., ~., starts = c(-3.69, 10.87, -8.31, 3.7120, 0.1185, 0.1436, 0.2408, 0.0, 0.0, 12.8)) 
...
## Now predict with top model
newdat <- data.frame(Pred.Occur = Pred.Occur.seq)
density.pred.pred.occur2.gdist <- predict(gdist.models$observer_date_timeXpred.occur2, type = "lambda", newdata = newdat, appendData=TRUE); density.pred.pred.occur2.gdist
...
## Check the goodness of fit of the best model and export summary
# Goodness of fit
pb.gdist <- parboot(gdist.models$observer_date_timeXpred.occur2, fitstats, nsim=100, report=5); pb.gdist # Can't do this apparently because of the issues in the model
...
capture.output(summary(gdist.models$observer_date_timeXpred.occur2), file = file.path(output.directory,filename))

# Detection probability for all bins within the 300m radius
p.bins <- getP(gdist.models$observer_date_timeXpred.occur2)
head(p.bins) # This shows that the detection probability increases then decreases. This isn't right. I may have to change the bins to fix this. 

p.gdistsamp <- mean(rowSums(getP(gdist.models$observer_date_timeXpred.occur2))); p.gdistsamp

test <- predict(gdist.models$observer_date_timeXpred.occur2, type = "det", newdata = umf, appendData=FALSE)
head(test) # These are the sigmas that affect the slope of the detection function
...
Rem.models$observer_date_timeXmarsh165 <- multinomPois(~ Observer + JulDat + Time ~ marsh3334_utm10_165, umf.rem)
summary(Rem.models$observer_date_timeXmarsh165)

Rem.models$observer_date_timeXmarsh315 <- multinomPois(~ Observer + JulDat + Time ~ marsh3334_utm10_315, umf.rem)
summary(Rem.models$observer_date_timeXmarsh315)

Rem.models$observer_date_timeXmarsh615 <- multinomPois(~ Observer + JulDat + Time ~ marsh3334_utm10_615, umf.rem)
summary(Rem.models$observer_date_timeXmarsh615)

Rem.models$observer_date_timeXmarsh1215 <- multinomPois(~ Observer + JulDat + Time ~ marsh3334_utm10_1215, umf.rem)
summary(Rem.models$observer_date_timeXmarsh1215)
...
## Finally, try to add other variables to habitat suitability to see if there are improvements
...
## Now predict the top model with mean suitability and a range of wvups. The max wvups is about 15, so going to 100 may be an issue
#wvups165.seq <- seq(0,100,10)
#newdat2 <- data.frame(wvups22_utm10_165 = wvups165.seq)
#newdat2$Pred.Occur <- 0.37609
#head(newdat2)

#abundance.pred.pred.occur2.wvups165.rem2 <- predict(Rem.models$date_hwy2Xpred.occur2_wvups165, type = "state", newdata = newdat2, appendData=TRUE); abundance.pred.pred.occur2.wvups165.rem2
...
gRem.models$observer_date_timeXriparian315 <- gmultmix(~ riparian2531_utm10_315, ~ 1, ~ Observer + JulDat + Time, umf.grem, mixture="NB", K=10, starts = coef(gRem.models$observer_date_timeXriparian165))
...
gRem.models$observer_date_timeXmarsh165 <- gmultmix(~ marsh3334_utm10_165, ~ 1, ~ Observer + JulDat + Time, umf.grem, mixture="NB", K=10)
summary(gRem.models$observer_date_timeXmarsh165)

gRem.models$observer_date_timeXmarsh315 <- gmultmix(~ marsh3334_utm10_315, ~ 1, ~ Observer + JulDat + Time, umf.grem, mixture="NB", K=10)
summary(gRem.models$observer_date_timeXmarsh315)

gRem.models$observer_date_timeXmarsh615 <- gmultmix(~ marsh3334_utm10_615, ~ 1, ~ Observer + JulDat + Time, umf.grem, mixture="NB", K=10)
summary(gRem.models$observer_date_timeXmarsh615)

gRem.models$observer_date_timeXmarsh1215 <- gmultmix(~ marsh3334_utm10_1215, ~ 1, ~ Observer + JulDat + Time, umf.grem, mixture="NB", K=10)
summary(gRem.models$observer_date_timeXmarsh1215)
...
gRem.models$observer_date_timeXpred.occur2 <- gmultmix(~ Pred.Occur + I(Pred.Occur^2), ~ 1, ~ Observer + JulDat + Time, umf.grem, mixture="NB", K=10)
...
gRem.models$observer_dateXpred.occur2 <- gmultmix(~ Pred.Occur + I(Pred.Occur^2), ~ 1, ~ Observer + JulDat, umf.grem, mixture="NB", starts = c(-3.18, 11.08, -8.38, -2.665, 0.677, 1.193, 2.695, 0.0, 12.2))
summary(gRem.models$observer_dateXpred.occur2)

gRem.models$observer_timeXpred.occur2 <- gmultmix(~ Pred.Occur + I(Pred.Occur^2), ~ 1, ~ Observer + Time, umf.grem, mixture="NB", starts = c(-3.18, 11.08, -8.38, -2.665, 0.677, 1.193, 2.695, 0.0, 12.2))
...
TOD.models$observer_date_timeXriparian165 <- multinomPois(~ Observer + JulDat + Time ~ riparian2531_utm10_165, umf.tod)
summary(TOD.models$observer_date_timeXriparian165)

TOD.models$observer_date_timeXriparian315 <- multinomPois(~ Observer + JulDat + Time ~ riparian2531_utm10_315, umf.tod)
summary(TOD.models$observer_date_timeXriparian315)

TOD.models$observer_date_timeXriparian615 <- multinomPois(~ Observer + JulDat + Time ~ riparian2531_utm10_615, umf.tod)
summary(TOD.models$observer_date_timeXriparian615)

TOD.models$observer_date_timeXriparian1215 <- multinomPois(~ Observer + JulDat + Time ~ riparian2531_utm10_1215, umf.tod)
summary(TOD.models$observer_date_timeXriparian1215)
...
## Riparian
gTOD.models$observer_date_timeXriparian165 <- gmultmix(~ riparian2531_utm10_165, ~ 1, ~ Observer + JulDat + Time, umf.gtod, mixture="NB", K=10)
summary(gTOD.models$observer_date_timeXriparian165)

gTOD.models$observer_date_timeXriparian315 <- gmultmix(~ riparian2531_utm10_315, ~ 1, ~ Observer + JulDat + Time, umf.gtod, mixture="NB", K=10)
summary(gTOD.models$observer_date_timeXriparian315)

gTOD.models$observer_date_timeXriparian615 <- gmultmix(~ riparian2531_utm10_615, ~ 1, ~ Observer + JulDat + Time, umf.gtod, mixture="NB", K=10)
summary(gTOD.models$observer_date_timeXriparian615)

gTOD.models$observer_date_timeXriparian1215 <- gmultmix(~ riparian2531_utm10_1215, ~ 1, ~ Observer + JulDat + Time, umf.gtod, mixture="NB", K=10)
summary(gTOD.models$observer_date_timeXriparian1215)
...
gTOD.models$observer_date_timeXpred.occur2 <- gmultmix(~ Pred.Occur + I(Pred.Occur^2), ~ 1, ~ Observer + JulDat + Time, umf.gtod, mixture="NB", K=10, starts = c(-3.29, 11.07, -8.40, -1.1206, -1.2921, 0.0696, 1.3331, 0.3183, -0.364, 11.8))
...
gTOD.models$observerXpred.occur2 <- gmultmix(~ Pred.Occur + I(Pred.Occur^2), ~ 1, ~ Observer, umf.gtod, mixture="NB", K=10, starts = c(-3.28, 9.71, -6.72, -1.0166, -1.3756, -0.0417, 1.2893, 11.8))
...
gTOD.models$observer_dateXpred.occur2 <- gmultmix(~ Pred.Occur + I(Pred.Occur^2), ~ 1, ~ Observer + JulDat, umf.gtod, mixture="NB", K=10, starts = c(-3.29, 11.11, -8.41, -0.154, -1.711, -0.168, 1.130, 0.345, 11.8))
...
current.preds.zi.raster[current.preds.zi.raster>calibration_cutoff] <- 1
current.preds.zi.raster[current.preds.zi.raster<=calibration_cutoff] <- 0
plot(current.preds.zi.raster)

writeRaster(current.preds.zi.raster, file.path(output.directory, paste(model.name,"_current_01suitable.tif", sep="")), 
            format="GTiff", overwrite=TRUE) 
writeRaster(current.preds.zi.raster, file.path(output.directory, paste(model.name,"_current_01suitable.asc", sep="")), 
            format="ascii", overwrite=TRUE) 
...
# Convert to 1 0 for suitable habitat
historic.preds.zi.raster[historic.preds.zi.raster>calibration_cutoff] <- 1
historic.preds.zi.raster[historic.preds.zi.raster<=calibration_cutoff] <- 0
plot(historic.preds.zi.raster)

writeRaster(historic.preds.zi.raster, file.path(output.directory, paste(model.name,"_historic_01suitable.tif", sep="")), 
            format="GTiff", overwrite=TRUE) 
writeRaster(historic.preds.zi.raster, file.path(output.directory, paste(model.name,"_historic_01suitable.asc", sep="")), 
            format="ascii", overwrite=TRUE) # Need to change the path here
...
## Current
current.suitable.habitat <- mask(current.preds.zi.raster, willamette_shape)
...
## Historic
historic.suitable.habitat <- mask(historic.preds.zi.raster, willamette_shape)
...
current_suitable <- shapefile(file.path(output.directory, "yewa_suitable_poly_current_clipped.shp")) # This is the polygon of just suitable areas
...
historic_suitable <- shapefile(file.path(output.directory, "yewa_suitable_poly_historic_clipped.shp")) # This is the polygon of just suitable areas
...
distsamp.density.current <- predict(dist.models$timeXpred.occur2, type = "state", newdata = current.stack) 

writeRaster(subset(distsamp.density.current,subset=1), file.path(output.directory, "yewa_distsamp_current_density180.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(subset(distsamp.density.current,subset=2), file.path(output.directory, "yewa_distsamp_current_density180_se.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(subset(distsamp.density.current,subset=3), file.path(output.directory, "yewa_distsamp_current_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(subset(distsamp.density.current,subset=4), file.path(output.directory, "yewa_distsamp_current_density180_upper.tif"), format="GTiff", overwrite=TRUE) 


distsamp.density.historic <- predict(dist.models$observer_date_timeXpred.occur2, type = "state", newdata = historic.stack)

writeRaster(subset(distsamp.density.historic,subset=1), file.path(output.directory, "yewa_distsamp_historic_density180.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(subset(distsamp.density.historic,subset=2), file.path(output.directory, "yewa_distsamp_historic_density180_se.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(subset(distsamp.density.historic,subset=3), file.path(output.directory, "yewa_distsamp_historic_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(subset(distsamp.density.historic,subset=4), file.path(output.directory, "yewa_distsamp_historic_density180_upper.tif"), format="GTiff", overwrite=TRUE) 


### Density with gdistsamp (negative binomial) top model
#gdistsamp.density.current <- predict(gdist.models$timeXpred.occur2, type = "lambda", newdata = current.stack) 

#writeRaster(subset(gdistsamp.density.current,subset=1), file.path(output.directory, "yewa_gdistsamp_current_density180.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(subset(gdistsamp.density.current,subset=2), file.path(output.directory, "yewa_gdistsamp_current_density180_se.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(subset(gdistsamp.density.current,subset=3), file.path(output.directory, "yewa_gdistsamp_current_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(subset(gdistsamp.density.current,subset=4), file.path(output.directory, "yewa_gdistsamp_current_density180_upper.tif"), format="GTiff", overwrite=TRUE) 

#gdistsamp.density.historic <- predict(gdist.models$observer_date_timeXpred.occur2, type = "lambda", newdata = historic.stack) 

#writeRaster(subset(gdistsamp.density.historic,subset=1), file.path(output.directory, "yewa_gdistsamp_historic_density180.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(subset(gdistsamp.density.historic,subset=2), file.path(output.directory, "yewa_gdistsamp_historic_density180_se.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(subset(gdistsamp.density.historic,subset=3), file.path(output.directory, "yewa_gdistsamp_historic_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(subset(gdistsamp.density.historic,subset=4), file.path(output.directory, "yewa_gdistsamp_historic_density180_upper.tif"), format="GTiff", overwrite=TRUE)
...
rem.abundance.current <- predict(Rem.models$observer_date_timeXpred.occur2, type = "state", newdata = current.stack) 
...
writeRaster(prediction, file.path(output.directory, "yewa_rem_current_density180.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(se, file.path(output.directory, "yewa_rem_current_density180_se.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(lower, file.path(output.directory, "yewa_rem_current_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(upper, file.path(output.directory, "yewa_rem_current_density180_upper.tif"), format="GTiff", overwrite=TRUE)

rem.abundance.historic <- predict(Rem.models$observer_date_timeXpred.occur2, type = "state", newdata = historic.stack) 
...
writeRaster(prediction, file.path(output.directory, "yewa_rem_historic_density180.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(se, file.path(output.directory, "yewa_rem_historic_density180_se.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(lower, file.path(output.directory, "yewa_rem_historic_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(upper, file.path(output.directory, "yewa_rem_historic_density180_upper.tif"), format="GTiff", overwrite=TRUE)


### Density with negative binomial removal top model
#grem.abundance.current <- predict(gRem.models$date_observer_timeXpred.occur2, type = "lambda", newdata = current.stack) 
...
#writeRaster(prediction, file.path(output.directory, "yewa_grem_current_density180.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(se, file.path(output.directory, "yewa_grem_current_density180_se.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(lower, file.path(output.directory, "yewa_grem_current_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(upper, file.path(output.directory, "yewa_grem_current_density180_upper.tif"), format="GTiff", overwrite=TRUE)

#grem.abundance.historic <- predict(gRem.models$date_observer_timeXpred.occur2, type = "lambda", newdata = historic.stack)
...
#writeRaster(prediction, file.path(output.directory, "yewa_grem_historic_density180.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(se, file.path(output.directory, "yewa_grem_historic_density180_se.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(lower, file.path(output.directory, "yewa_grem_historic_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(upper, file.path(output.directory, "yewa_grem_historic_density180_upper.tif"), format="GTiff", overwrite=TRUE)


### Density with time of detection top model
#tod.density.current <- predict(TOD.models$date_time_observerXpred.occur2, type = "state", newdata = current.stack) 
...
#writeRaster(prediction, file.path(output.directory, "yewa_tod_current_density180.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(se, file.path(output.directory, "yewa_tod_current_density180_se.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(lower, file.path(output.directory, "yewa_tod_current_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(upper, file.path(output.directory, "yewa_tod_current_density180_upper.tif"), format="GTiff", overwrite=TRUE)

#tod.density.historic <- predict(TOD.models$date_time_observerXpred.occur2, type = "state", newdata = historic.stack) 
...
#writeRaster(prediction, file.path(output.directory, "yewa_tod_historic_density180.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(se, file.path(output.directory, "yewa_tod_historic_density180_se.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(lower, file.path(output.directory, "yewa_tod_historic_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
#writeRaster(upper, file.path(output.directory, "yewa_tod_historic_density180_upper.tif"), format="GTiff", overwrite=TRUE)



## Density with negative binomial time of detection top model
gtod.density.current <- predict(gTOD.models$observer_date_timeXpred.occur2, type = "lambda", newdata = current.stack) 
...
writeRaster(prediction, file.path(output.directory, "yewa_gtod_current_density180.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(se, file.path(output.directory, "yewa_gtod_current_density180_se.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(lower, file.path(output.directory, "yewa_gtod_current_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(upper, file.path(output.directory, "yewa_gtod_current_density180_upper.tif"), format="GTiff", overwrite=TRUE)

gtod.density.historic <- predict(gTOD.models$observer_date_timeXpred.occur2, type = "lambda", newdata = historic.stack) 
...
writeRaster(prediction, file.path(output.directory, "yewa_gtod_historic_density180.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(se, file.path(output.directory, "yewa_gtod_historic_density180_se.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(lower, file.path(output.directory, "yewa_gtod_historic_density180_lower.tif"), format="GTiff", overwrite=TRUE) 
writeRaster(upper, file.path(output.directory, "yewa_gtod_historic_density180_upper.tif"), format="GTiff", overwrite=TRUE)
...
## distsamp
# Current
current.dens.prediction <- raster(file.path(output.directory, "yewa_distsamp_current_density180.tif")) 
...
current.dens.lower <- raster(file.path(output.directory, "yewa_distsamp_current_density180_lower.tif")) 
...
current.dens.upper <- raster(file.path(output.directory, "yewa_distsamp_current_density180_upper.tif")) 
...
# Historic
historic.dens.prediction <- raster(file.path(output.directory, "yewa_distsamp_historic_density180.tif")) 
...
historic.dens.lower <- raster(file.path(output.directory, "yewa_distsamp_historic_density180_lower.tif")) 
...
historic.dens.upper <- raster(file.path(output.directory, "yewa_distsamp_historic_density180_upper.tif")) 
...
### gdistsamp
## Current
#current.dens.prediction <- raster(file.path(output.directory, "yewa_gdistsamp_current_density180.tif")) 
...
#current.dens.lower <- raster(file.path(output.directory, "yewa_gdistsamp_current_density180_lower.tif")) 
...
#current.dens.upper <- raster(file.path(output.directory, "yewa_gdistsamp_current_density180_upper.tif")) 
...
## Historic
#historic.dens.prediction <- raster(file.path(output.directory, "yewa_gdistsamp_historic_density180.tif")) 
...
#historic.dens.lower <- raster(file.path(output.directory, "yewa_gdistsamp_historic_density180_lower.tif")) 
...
#historic.dens.upper <- raster(file.path(output.directory, "yewa_gdistsamp_historic_density180_upper.tif")) 
...
## rem
# Current
current.dens.prediction <- raster(file.path(output.directory, "yewa_rem_current_density180.tif")) 
...
current.dens.lower <- raster(file.path(output.directory, "yewa_rem_current_density180_lower.tif")) 
...
current.dens.upper <- raster(file.path(output.directory, "yewa_rem_current_density180_upper.tif")) 
...
# Historic
historic.dens.prediction <- raster(file.path(output.directory, "yewa_rem_historic_density180.tif")) 
...
historic.dens.lower <- raster(file.path(output.directory, "yewa_rem_historic_density180_lower.tif")) 
...
historic.dens.upper <- raster(file.path(output.directory, "yewa_rem_historic_density180_upper.tif")) 
...
### grem
## Current
#current.dens.prediction <- raster(file.path(output.directory, "yewa_grem_current_density180.tif")) 
...
#current.dens.lower <- raster(file.path(output.directory, "yewa_grem_current_density180_lower.tif")) 
...
#current.dens.upper <- raster(file.path(output.directory, "yewa_grem_current_density180_upper.tif")) 
...
## Historic
#historic.dens.prediction <- raster(file.path(output.directory, "yewa_grem_historic_density180.tif")) 
...
#historic.dens.lower <- raster(file.path(output.directory, "yewa_grem_historic_density180_lower.tif")) 
...
#historic.dens.upper <- raster(file.path(output.directory, "yewa_grem_historic_density180_upper.tif")) 
...
### tod
## Current
#current.dens.prediction <- raster(file.path(output.directory, "yewa_tod_current_density180.tif")) 
...
#current.dens.lower <- raster(file.path(output.directory, "yewa_tod_current_density180_lower.tif")) 
...
#current.dens.upper <- raster(file.path(output.directory, "yewa_tod_current_density180_upper.tif")) 
...
## Historic
#historic.dens.prediction <- raster(file.path(output.directory, "yewa_tod_historic_density180.tif")) 
...
#historic.dens.lower <- raster(file.path(output.directory, "yewa_tod_historic_density180_lower.tif")) 
...
#historic.dens.upper <- raster(file.path(output.directory, "yewa_tod_historic_density180_upper.tif")) 
...
## gtod
# Current
current.dens.prediction <- raster(file.path(output.directory, "yewa_gtod_current_density180.tif")) 
...
current.dens.lower <- raster(file.path(output.directory, "yewa_gtod_current_density180_lower.tif")) 
...
current.dens.upper <- raster(file.path(output.directory, "yewa_gtod_current_density180_upper.tif")) 
...
# Historic
historic.dens.prediction <- raster(file.path(output.directory, "yewa_gtod_historic_density180.tif")) 
...
historic.dens.lower <- raster(file.path(output.directory, "yewa_gtod_historic_density180_lower.tif")) 
...
historic.dens.upper <- raster(file.path(output.directory, "yewa_gtod_historic_density180_upper.tif")) 
...
Counts_PredOccur.df <- cbind(site.counts, covs)
names(Counts_PredOccur.df)

sa.plot <- ggscatter(Counts_PredOccur.df, x = "Pred.Occur", y = "site.counts",
                                #add = "reg.line",                                 # Add regression line
                                #conf.int = TRUE,                                  # Add confidence interval
                                ylab = "Observed Abundance", xlab = "Habitat Suitability",
                                add.params = list(color = "blue",
                                                  fill = "lightgray")
) ; sa.plot

lm1 <- lm(site.counts ~ Pred.Occur, data = Counts_PredOccur.df)
summary(lm1)

lm2 <- lm(site.counts ~ Pred.Occur + I(Pred.Occur^2), data = Counts_PredOccur.df)
summary(lm2)
